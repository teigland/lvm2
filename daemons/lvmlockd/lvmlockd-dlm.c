...
int lm_lock_dlm(struct lockspace *ls, struct resource *r, int ld_mode, uint64_t *r_version)
{
	struct lm_dlm *lmd = ls->lm_data;
	struct rd_dlm *rdd;
	struct dlm_lksb *lksb;
	...
	if (rdd->vb) {
		...
		*r_version = le64_to_cpu(vb.r_version);
		...
		log_debug("S %s R %s lock_dlm get r_version %llu",
		   ...
		   int ld_mode, uint64_t r_version)
{
	...
	if (rdd->vb && r_version && (r->mode == LD_LK_EX)) {
		...
		rdd->vb->r_version = cpu_to_le64(r_version);
		memcpy(lksb->sb_lvbptr, rdd->vb, sizeof(struct val_blk));

		log_debug("S %s R %s convert_dlm set r_version %llu",
			  ls->name, r->name, (unsigned long long)r_version);
		  ...
		  uint64_t r_version, uint32_t lmuf_flags)
{
	...
	log_debug("S %s R %s unlock_dlm r_version %llu flags %x",
	...
	if (rdd->vb && r_version && (r->mode == LD_LK_EX)) {
		...
		log_debug("S %s R %s unlock_dlm set r_version %llu",
			  ls->name, r->name, (unsigned long long)r_version);
