...
static void read_lvb(struct resource *r, struct dlm_lksb *lksb, uint64_t *r_version)
{
	struct val_blk vb;
	uint32_t version;

	if (lksb->sb_flags & DLM_SBF_VALNOTVALID) {
		log_error("R %s VALNOTVALID", r->name);
		*r_version = 0;
		return;
	}
...
static void write_lvb(struct resource *r, struct dlm_lksb *lksb, uint64_t r_version)
{
	struct val_blk vb;
...
int lm_unlock_dlm(struct lockspace *ls, struct resource *r,
		  uint64_t r_version, uint32_t lm_uf_flags)
{
	struct lm_dlm *lmd = ls->lm_data;
	struct dlm_lksb *lksb = r->lm_data;
	uint32_t flags;
