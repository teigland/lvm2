...
int lm_init_vg_sanlock(char *ls_name, uint32_t flags, char *vg_args)
{
	struct sanlk_lockspace ss;
	...
	strncpy(disk.path, vg_args, SANLK_PATH_LEN);
	...
	log_debug("init_vg_sanlock done %s %s %s", ls_name, gl_name, disk.path);
		       ...
		       char *vg_args, char *lv_args)
{
	...
	rd.rs.num_disks = 1;
	strncpy(rd.rs.disks[0].path, vg_args, SANLK_PATH_LEN);
	...
	while (1) {
		...
		rv = sanlock_write_resource(&rd.rs, 0, 0, 0);
		if (!rv)
			sprintf(lv_args, "%llu", (unsigned long long)offset);
		else
			  ...
			  uint64_t *lv_args_offset)
{
	...
	rd.rs.num_disks = 1;
	strncpy(rd.rs.disks[0].path, ls->vg_args, SANLK_PATH_LEN);
...
int lm_add_lockspace_sanlock(struct lockspace *ls)
{
	...
	strncpy(lms->ss.host_id_disk.path, ls->vg_args, SANLK_PATH_LEN);
...
/*
 * lv_args is only needed for lv locks; it is the offset that lm_init_lv
 * used above, i.e. the disk offset where the lease for this lv resides.
 * (gl and vg locks exist at fixed, known offsets).
 *
 * If lv_args for lv was not provided, we try a linear search to find
				   ...
				   char *lv_args)
{
	...
	if (r->type == LD_RT_LV) {
		if (!strcmp(lv_args, "none")) {
			rv = find_lv_offset(ls, r, &lv_args_offset);
			if (rv < 0)
				return rv;
		} else {
			lv_args_offset = atoll(lv_args);
		}
	}
	...
	if (r->type == LD_RT_GL)
		...
		rds->rs.disks[0].offset = lv_args_offset;
...
int lm_lock_sanlock(struct lockspace *ls, struct resource *r,
		    int ld_mode, char *lv_args, uint64_t *version)
{
	struct lm_sanlock *lms = ls->lm_data;
	struct rd_sanlock *rds = r->lm_data;
		       ...
		       int ld_mode, uint64_t version)
{
	...
	/* sanlock_convert not yet in released sanlock package */
#if 0
