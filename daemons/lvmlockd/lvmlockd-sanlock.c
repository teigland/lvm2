				   ...
				   char *lv_args)
{
	...
	if (r->type == LD_RT_LV) {
		if (!lv_args[0] || !strcmp(lv_args, "none")) {
			...
			if (rv < 0) {
				log_error("S %s R %s add_resource_san bad offset from args",
					  ls->name, r->name);
				return rv;
			}
		}
	...
	log_debug("S %s R %s add_resource_san offset %llu",
		  ls->name, r->name,
		    ...
		    int ld_mode, char *lv_args, uint64_t *val)
{
	...
	if (rv == -EAGAIN) {
		...
		log_error("S %s R %s lock_san EAGAIN", ls->name, r->name);
		return -EAGAIN;
	}
	if (rv < 0) {
	...
	if (lvb) {
		...
		if (rv < 0) {
			log_error("S %s R %s lock_san get_lvb error %d",
				  ls->name, r->name, rv);
			*val = 0;
			return rv;
		}
		...
		if ((version & 0xFFFF0000) > (VAL_BLK_VERSION & 0xFFFF0000)) {
			log_error("lock_san ignoring incompatible val blk version %x", version);
		} else {
			*val = le64_to_cpu(vb.val);

			log_debug("S %s R %s lock_san get version %llu",
				  ls->name, r->name, (unsigned long long)(*val));
		}
	}
		       ...
		       int ld_mode, uint64_t val)
{
	...
	if (lvb && (r->mode == LD_LK_EX)) {
		...
		log_debug("S %s R %s convert_san set val %llu",
			  ls->name, r->name, (unsigned long long)val);
 ...
 rs_flag:
	...
	if (rv < 0) {
		log_error("S %s R %s convert_san convert error %d",
			  ls->name, r->name, rv);
	}
...
static int release_rename(struct lockspace *ls, struct resource *r)
{
	...
	if (rv < 0) {
		log_error("S %s R %s unlock_san release rename error %d",
			  ls->name, r->name, rv);
	}
...
int lm_unlock_sanlock(struct lockspace *ls, struct resource *r,
		      uint64_t val, uint32_t lmuf_flags)
{
	struct lm_sanlock *lms = ls->lm_data;
	struct rd_sanlock *rds = r->lm_data;
	struct sanlk_resource *rs = &rds->rs;
	...
	log_debug("S %s R %s unlock_san val %llu flags %x",
		  ls->name, r->name, (unsigned long long)val, lmuf_flags);

	if (daemon_test)
		return 0;

	if (lvb && val && (r->mode == LD_LK_EX)) {
		memset(&vb, 0, sizeof(vb));

		vb.version = cpu_to_le32(VAL_BLK_VERSION);
		vb.val = cpu_to_le64(val);

		log_debug("S %s R %s unlock_san set val %llu",
			  ls->name, r->name, (unsigned long long)val);
	...
	if (rv < 0) {
		log_error("S %s R %s unlock_san release error %d",
			  ls->name, r->name, rv);
	}
...
int lm_hosts_sanlock(struct lockspace *ls, int notify)
{
	...
	if (found_others && notify) {
		...
		hm.host_id = 0xFFFFFFFF;
		hm.generation = 0xFFFFFFFF;
		hm.msg = SANLK_HM_LVM_CHECK | SANLK_HM_SEQ_DATA;
		hm.seq = 1;
		sanlock_set_message(ls->name, 0, sizeof(hm), hm);
#endif
		/* TODO: should we wait here for others to get the message and stop
		 * their lockspace before checking again? */
	}
