...
int lm_lock_sanlock(struct lockspace *ls, struct resource *r,
		    int ld_mode, char *lv_args, uint32_t *r_version, int *retry)
{
	struct lm_sanlock *lms = ls->lm_data;
	struct rd_sanlock *rds;
	struct sanlk_resource *rs;
	...
	if (rds->vb) {
		...
		*r_version = le32_to_cpu(vb.r_version);
		...
		log_debug("S %s R %s lock_san get r_version %u", ls->name, r->name, *r_version);
	}
out:
	return rv;
}
		       ...
		       int ld_mode, uint32_t r_version)
{
	...
	if (rds->vb && r_version && (r->mode == LD_LK_EX)) {
		...
		log_debug("S %s R %s convert_san set r_version %u", ls->name, r->name, r_version);
