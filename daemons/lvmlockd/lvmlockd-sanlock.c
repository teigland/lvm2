...
int lm_lock_sanlock(struct lockspace *ls, struct resource *r,
		    int ld_mode, char *lv_args, uint64_t *r_version, int *retry)
{
	struct lm_sanlock *lms = ls->lm_data;
	struct rd_sanlock *rds;
	struct sanlk_resource *rs;
	...
	if (lvb) {
		...
		/* TODO: keep vb read copy so version field can be selectively modified
		   later in lm_unlock/lm_convert*/
		...
		if ((version & 0xFFFF0000) > (VAL_BLK_VERSION & 0xFFFF0000)) {
			...
			*r_version = le64_to_cpu(vb.r_version);

			log_debug("S %s R %s lock_san get r_version %llu",
				  ls->name, r->name, (unsigned long long)(*r_version));
		}
	}
...
int lm_convert_sanlock(struct lockspace *ls, struct resource *r,
		       int ld_mode, uint64_t r_version)
{
	struct lm_sanlock *lms = ls->lm_data;
	struct rd_sanlock *rds = r->lm_data;
	struct sanlk_resource *rs = &rds->rs;
	...
	if (lvb && r_version && (r->mode == LD_LK_EX)) {
		/* TODO: keep last vb read copy and just modify r_version */
		memset(&vb, 0, sizeof(vb));

		vb.version = cpu_to_le32(VAL_BLK_VERSION);
		vb.r_version = cpu_to_le64(r_version);

		log_debug("S %s R %s convert_san set r_version %llu",
			  ls->name, r->name, (unsigned long long)r_version);
...
int lm_unlock_sanlock(struct lockspace *ls, struct resource *r,
		      uint64_t r_version, uint32_t lmuf_flags)
{
	struct lm_sanlock *lms = ls->lm_data;
	struct rd_sanlock *rds = r->lm_data;
	struct sanlk_resource *rs = &rds->rs;
	...
	log_debug("S %s R %s unlock_san r_version %llu flags %x",
		  ls->name, r->name, (unsigned long long)r_version, lmuf_flags);
	...
	if (lvb && r_version && (r->mode == LD_LK_EX)) {
		...
		log_debug("S %s R %s unlock_san set r_version %llu",
			  ls->name, r->name, (unsigned long long)r_version);
