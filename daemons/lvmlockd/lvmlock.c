...
static char *vg_mode;
static char *lv_mode;
static char *lock_mode; /* one of the above */
...
static void print_usage(void)
{
	...
	printf(". for sanlock, this returns a new offset that must\n");
	printf("  be used as lm_args when acquiring the lv lock\n");
	printf(". for dlm, lock-init does nothing\n");
	printf("\n");
	...
	printf(". lm_args is the string that was returned by lock-init\n");
	printf("\n");
	...
	*/
	...
	while (1) {
		...
		case 'c':
			...
			/* TODO: args from remaining options */
			break;
		default:
			print_usage();
			exit(1);
...
int main(int argc, char **argv)
{
	...
	if (update && (lock_gl || lock_vg) && !strcmp(lock_mode, "ex")) {
		rv = do_update();
		log_debug("update %d", rv);
	}
