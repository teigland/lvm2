...
static int do_test(void)
{
	...
	reply = daemon_send_simple(_lvmlockd_handle, "test",
			       ...
			       "vg_lock_type = %s", lm_type ?: "test",
			       "vg_lock_args = %s", lm_args ?: "test",
			       NULL);
...
static int do_lock_init_vg(void)
{
	...
	reply = daemon_send_simple(_lvmlockd_handle,
				"init_vg",
				"vg_name = %s", vg_name,
				"vg_lock_type = %s", lm_type,
				"vg_lock_args = %s", lm_args,
				NULL);
...
static int do_lock_start_vg(void)
{
	...
	if (!lm_type) {
		/* lvmlockd will try to get lock_type/lock_args/vg_uuid from lvmetad */
		reply = daemon_send_simple(_lvmlockd_handle,
					"start_vg",
					"vg_name = %s", vg_name,
					"host_id = %s", host_id,
