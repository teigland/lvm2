...
/*
 * VG's that do not have a dlock type, i.e. "none", "local".
 ...
 */
static struct list_head local_vgs_mutex;
		   ...
		   int mode, struct action *act, uint64_t *version)
{
	if (ls->lm_type == LD_LM_DLM)
		...
		return lm_lock_sanlock(ls, r, mode, act->lv_args, version);
	return -1;
}
...
do_unlock:
	 ...
	 * step.  Or, we might find a reason to implement
	...
	else if (r->type == LD_RT_GL && r->mode == LD_LK_EX)
		r->version++;
...
static void get_vg_lm_info(struct lockspace *ls)
{
	...
	if (strcmp(reply_response, "OK")) {
		log_error("get_vg_lm_info %s lvmetad response %s", ls->name, reply_response);
		goto out;
	}
	...
	if (!ls->vg_name[0]) {
		str = dm_config_find_str(metadata, "name", NULL);
		if (str)
	...
	if (!lock_type) {
		log_error("get_vg_lm_info %s empty lock_type", ls->name);
		goto out;
	}
...
static void *lockspace_thread_main(void *arg_in)
{
	...
	log_debug("L %s add %d", ls->name, error);
	...
	while (1) {
		...
		while (1) {
			...
			log_debug("L %s R %s op %s %s", ls->name, r->name,
...
out_rem:
	...
	log_debug("L %s remove done", ls->name);
...
/* local_vgs_mutex is locked */
static void add_local_vg(const char *vg_name, const char *vg_uuid)
{
	struct lockspace *ls;
...
static int start_lockspaces(uint64_t host_id)
{
	...
	if (!(cn = dm_config_find_node(reply.cft->root, "volume_groups"))) {
		log_error("start_lockspaces no vg data from lvmetad");
		return -ENOENT;
	}
	...
	for (cn = cn->child; cn; cn = cn->sib) {
		...
		if (!lm_type) {
			pthread_mutex_lock(&local_vgs_mutex);
			add_local_vg(lockspace.vg_name, vg_uuid);
			pthread_mutex_unlock(&local_vgs_mutex);
			continue;
		}
		...
		rv = add_lockspace_thread(ls_name, lockspace.vg_name, lockspace.vg_uuid,
					  ...
					  NULL);
		if (rv)
			log_error("start_lockspaces %s add_lockspace_thread %d", ls_name, rv);
	}
...
static int init_vg(struct action *act)
{
	...
	if (act->lm_type == LD_LM_SANLOCK)
		rv = lm_init_vg_sanlock(ls_name, act->flags, act->vg_args);
	else if (act->lm_type == LD_LM_DLM)
		rv = lm_init_vg_dlm(ls_name, act->flags, act->vg_args);
	else
		rv = -EINVAL;
...
static void *worker_thread_main(void *arg_in)
{
	...
	while (1) {
		...
		if (act->flags & LD_AF_SEARCH_LS) {
			...
			if (is_local) {
				/* errno indicates the vg is local, see client_send_result. */
				act->result = -ENOTNAM;
				add_client_result(act);
				break;
			}
			...
			if (is_ls) {
				...
				/*
				 * TODO: whenever we return an error because the
				 * lockspace could not be found, we also want the
				 * reply to indicate whether:
				 * - no dlock_type lockspaces have been seen/found
				 * - dlock_type lockspaces are seen, but no gl lockspace
				 *
				 * With this info, the command can decide based on
				 * the situation (mode/command) whether it can
				 * go ahead without the lock.
				 *
				 * (Whether are the places where we return errors
				 * due to not finding the lockspace?)
				 */

				/* same reason that the action came here, so give up */
		...
		default:
			log_error("worker_thread: unknown op %d", act->op);
			act->result = -EINVAL;
			add_client_result(act);
			break;
		};
...
static void client_send_result(struct client *cl, struct action *act)
{
	...
	/*
	 * init_vg with dlm returns vg_args (cluster name)
	 * init_lv with sanlock returns lv_args (device offset)
	 */

	log_debug("C send %s [%d %u] %s %s rv %d",
		  cl->name[0] ? cl->name : "client", cl->pid, cl->id,
		  rt_str(act->rt), op_str(act->op) act->result);

	if (act->result == -ENOTNAM) {
		/*
		 * This magic error number means the vg was found to be local.
		 * The local_vg case is usually caught immediately in
		 * client_recv_action, but can sometimes be caught later,
		 * in which case we need to handle the reply here.
		 */
...
static int add_lock_action(struct action *act)
{
	...
	if (act->rt == LD_RT_GL && act->op == LD_OP_ENABLE && gl_use_sanlock) {
		...
		if (!ls_name[0]) {
			log_debug("add_lock_action no gl lockspace");

			/*
			 * pass act to worker thread to:
			 * - start_lockspaces
			 * - check if needed lockspace now exists
			 * - if so, requeue here
			 * - if not, return an error indicating:
			 *   . no dlock_type vgs have been seen
			 *   . dlock_type vgs are seen, but no gl lockspace exists
			 */
	...
	if (!ls) {
		 ...
		 * for these steps.  When it's done, it will requeue this
		 * action for the lockspace.
		 */
...
static void client_recv_action(struct client *cl)
{
	...
	pthread_mutex_lock(&local_vgs_mutex);
	if (find_local_vg(vg_name, vg_uuid)) {
		pthread_mutex_unlock(&local_vgs_mutex);

		/* This is an optimization for the common case of vg's that we
		   already know do not need any work. */ 
	...
	log_debug("C recv %s [%d %u] %s %s",
		  cl->name[0] ? cl->name : "client", cl->pid, cl->id,
		  rt_str(act->rt), op_str(act->op));
	...
	case LD_OP_ADD_LOCAL:
		pthread_mutex_lock(&local_vgs_mutex);
		add_local_vg(act->vg_name, act->vg_uuid);
		pthread_mutex_unlock(&local_vgs_mutex);
		act->result = 0;
		add_client_result(act);
		rv = 0;
...
out:
	if (rv < 0) {
		log_debug("op %s error %d", op_str(act->op), rv);
		act->result = rv;
		add_client_result(act);
	}
}
...
int main(int argc, char *argv[])
{
	...
	// use getopt_long
	while ((opt = getopt(argc, argv, "?fhVs:i:")) != EOF) {
		switch (opt) {
		case 'i':
