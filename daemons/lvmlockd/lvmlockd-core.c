...
static int res_lock(struct lockspace *ls, struct resource *r, struct action *act, int *retry)
{
	...
	if (r_version > r->version) {
		...
		if ((r->type == LD_RT_GL) && (n_version > ls->names_version)) {
			/*
			 * When gl is unlocked, UPDATE_LOCAL work action will be
			 * queued to update local_vgs.  It needs to happen on
			 * unlock because lvmetad needs to be updated by the
			 * command before we can get the updated vg list.
			 */
			ls->update_local = 1;
...
do_unlock:
	...
	if ((r->type == LD_RT_GL) && (r->mode == LD_LK_EX)) {
		...
		if (act->flags & LD_AF_VG_LIST_CHANGE) {
			ls->names_version++;
			n_version = ls->names_version;
			log_debug("S %s R %s res_unlock n_version inc %u", ls->name, r->name, n_version);
		}
	...
	if ((r->type == LD_RT_GL) && ls->update_local) {
		log_debug("S %s R %s res_unlock update local", ls->name, r->name);
		ls->update_local = 0;
 ...
 *
 * Alternatively, we could try to execute a 'vgchange --lock-start' command
 * from here.
 */

static void work_update_local_vgs(void)
{
...
next:
	...
	list_for_each_entry_safe(uls, safe, &update_vgs, list) {
		if (uls->lm_type != LD_LM_NONE)
			continue;
		/* add_local_vg doesn't add any that already exist */
				...
				uint64_t host_id, struct action *act)
{
	...
	log_debug("add_lockspace_thread %s %s version %u",
		  lm_str(lm_type), ls_name, act->version);
...
static int add_lockspace(struct action *act)
{
	...
	if (ls) {
		log_error("add_lockspace vg %s removing matching local_vg", act->vg_name);
		list_del(&ls->list);
...
static void *worker_thread_main(void *arg_in)
{
	...
	while (1) {
		...
		/*
		 * worker_thread serves as a helper to search existing
...
/*
 * We could possibly limit when these commands return 1 to
 * cases where they are run on local vgs.  These commands are
 * either not allowed on dlock-type vgs, or we already detect
 * them via their locks.
 */

static int command_changes_vg_list(const char *name)
{
	if (!strcmp(name, "vgcreate"))
		return 1;
	if (!strcmp(name, "vgremove"))
		return 1;
	if (!strcmp(name, "vgrename"))
		return 1;
	if (!strcmp(name, "vgmerge"))
		return 1;
	if (!strcmp(name, "vgsplit"))
		return 1;
	return 0;
}
...
static void client_purge(struct client *cl)
{
	...
	list_for_each_entry(ls, &lockspaces, list) {
		...
		/*
		 * If this close action results in gl unlock from ex,
		 * then this flag causes the an r->names_version++ 
		 * which will be written to the gl lvb n_version.
		 * Alternatively, we could save the command name in
		 * the act struct, get rid of the flag, and check the
		 * command name when doing gl ex unlock.
		 */
		if (command_changes_vg_list(cl->name))
			act->flags |= LD_AF_VG_LIST_CHANGE;
...
static int add_lock_action(struct action *act)
{
 ...
 retry:
	...
	if (!ls->create_fail && !ls->create_done) {
		pthread_mutex_unlock(&ls->mutex);
		pthread_mutex_unlock(&lockspaces_mutex);
...
static void client_recv_action(struct client *cl)
{
	...
	/* TODO: allow gl_use_dlm/gl_use_sanlock to be set as a daemon option
	   at startup to avoid the uncertainty if vgs with both types exist. */
...
/*
 * TODO: add an lvm.conf config option to allow
 * lvm commands and lvmlockd to get the local system id
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		...
		case 's':
			...
			break;
		case 'l':
			lvmlockd_log_config = strdup(optarg);
			break;
		case 'a':
			local_thread_also = 1;
			break;
