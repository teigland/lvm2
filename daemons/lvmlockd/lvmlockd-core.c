...
static int lm_lock(struct lockspace *ls, struct resource *r,
		   int mode, struct action *act, uint32_t *r_version, int *retry)
{
	int rv;

	if (ls->lm_type == LD_LM_DLM)
		rv = lm_lock_dlm(ls, r, mode, r_version);
	else if (ls->lm_type == LD_LM_SANLOCK)
		rv = lm_lock_sanlock(ls, r, mode, act->lv_args, r_version, retry);
	else
		return -1;
...
static int lm_unlock(struct lockspace *ls, struct resource *r,
		     struct action *act, uint32_t r_version, uint32_t lmuf_flags)
{
	int rv;

	if (ls->lm_type == LD_LM_DLM)
		return lm_unlock_dlm(ls, r, r_version, lmuf_flags);
	else if (ls->lm_type == LD_LM_SANLOCK)
		return lm_unlock_sanlock(ls, r, r_version, lmuf_flags);
	else
		return -1;
...
static int res_lock(struct lockspace *ls, struct resource *r, struct action *act, int *retry)
{
	...
	rv = lm_lock(ls, r, act->mode, act, &version, retry);
	if (rv == -EAGAIN)
		return rv;
	if (rv < 0) {
	...
	log_debug("S %s R %s res_lock lm done version %u", ls->name, r->name, version);
		       ...
		       struct lock *lk, struct action *act)
{
	 ...
	 */
		...
		version = r->version;
		log_debug("S %s R %s res_convert version inc %u", ls->name, r->name, version);

	} else if ((r->type == LD_RT_VG) && (r->mode == LD_LK_EX) && (lk->version > r->version)) {
		r->version = lk->version;
		version = r->version;
		log_debug("S %s R %s res_convert version new %u", ls->name, r->name, version);
	} else {
		version = 0;
	}

	rv = lm_convert(ls, r, act->mode, act, version);
	if (rv < 0) {
...
do_unlock:
	...
	/*
	 * lm_unlock() writes new version (from ex)
	 *
	...
	if ((r->type == LD_RT_GL) && (r->mode == LD_LK_EX)) {
		...
		lk->version = r->version;
		version = r->version;
		log_debug("S %s R %s res_unlock version inc %u", ls->name, r->name, version);

	} else if ((r->type == LD_RT_VG) && (r->mode == LD_LK_EX) && (lk->version > r->version)) {
		r->version = lk->version;
		version = r->version;
		log_debug("S %s R %s res_unlock version new %u", ls->name, r->name, version);
	} else {
		version = 0;
	}
...
static int clear_locks(struct lockspace *ls, int free_vg)
{
	...
	list_for_each_entry_safe(r, r_safe, &ls->resources, list) {
		...
		if ((r->type == LD_RT_GL) && (r->mode == LD_LK_EX)) {
			r->version++;
			version = r->version;
			log_debug("S %s R %s clear_locks version inc %u", ls->name, r->name, version);

		} else if ((r->type == LD_RT_VG) && (r->mode == LD_LK_EX) && (lk_version > r->version)) {
			r->version = lk_version;
			version = r->version;
			log_debug("S %s R %s clear_locks version new %u", ls->name, r->name, version);
