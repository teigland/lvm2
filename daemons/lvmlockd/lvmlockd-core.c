...
static void get_vg_lm_info(struct lockspace *ls)
{
	...
	if (strcmp(lock_type, "dlm"))
		...
		log_error("get_vg_lm_info %s unknown lock_type %s", ls->name, lock_type);
		goto out;
	}
...
static int start_lockspaces(uint64_t host_id)
{
	...
	for (cn = cn->child; cn; cn = cn->sib) {
		...
		if (!lockspace.lm_type)
			continue;

		if (lockspace.lm_type == LD_LM_DLM) {
			add_dlm_global_lockspace(NULL);
		}

		memset(ls_name, 0, sizeof(ls_name));

		rv = vg_ls_name(lockspace.vg_name, ls_name);
		if (rv) {
		}
...
static void client_send_result(struct client *cl, struct action *act)
{
	...
	log_debug("client_send_result op %d result %d", act->op, act->result);
