...
static void *worker_thread_main(void *arg_in)
{
	...
	while (1) {
		...
		if (act->flags & LD_AF_SEARCH_LS) {
			log_debug("worker: search %s lockspace vg \"%s\"",
			...
			if (is_local) {
				log_debug("worker: found local vg \"%s\"", act->vg_name);
				act->result = -ELOCALVG;
				add_client_result(act);
				continue;
			}
			...
			if (is_ls) {
				log_debug("worker: found %s lockspace \"%s\"",
				...
				log_error("worker: cannot find %s lockspace \"%s\"",
					  rt_str(act->rt), ls_name);
				act->result = -ENOLS;
				add_client_result(act);
			}
...
static void client_send_result(struct client *cl, struct action *act)
{
	...
	if (act->result == -ELOCALVG) {
		...
		log_debug("C send %s [%d %u] %s %s rv %d ELOCALVG %s",
		...
		log_debug("C send %s [%d %u] %s %s rv %d vg_args %s lv_args %s",
			  cl->name[0] ? cl->name : "client", cl->pid, cl->id,
			  op_str(act->op), rt_str(act->rt),
		...
		log_debug("C send %s [%d %u] %s %s rv %d %s result_flags %s",
			  cl->name[0] ? cl->name : "client", cl->pid, cl->id,
			  op_str(act->op), rt_str(act->rt),
