...
static int lm_lock(struct lockspace *ls, struct resource *r,
		   int mode, struct action *act, uint64_t *version, int *retry)
{
	int rv;
	...
	if (ls->lm_type == LD_LM_DLM)
		...
		rv = lm_lock_sanlock(ls, r, mode, act->lv_args, version, retry);
	else
		return -1;
		       ...
		       struct lock *lk, struct action *act)
{
	 ...
	 */
		...
		version = r->version;
		log_debug("S %s R %s res_convert version inc %llu", ls->name, r->name, (unsigned long long)version);

	} else if ((r->type == LD_RT_VG) && (r->mode == LD_LK_EX) && (lk->version > r->version)) {
		r->version = lk->version;
		version = r->version;
		log_debug("S %s R %s res_convert version new %llu", ls->name, r->name, (unsigned long long)version);
	} else {
		version = 0;
	}
				...
				uint64_t host_id, struct action *act)
{
	...
	log_debug("add_lockspace_thread %s %s version %llu",
		  lm_str(lm_type), ls_name, (unsigned long long)act->version);
