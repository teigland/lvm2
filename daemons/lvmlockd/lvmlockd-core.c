...
static time_t last_update_local;

static void work_remove_update_local(void)
{
	struct action *act, *safe;

	pthread_mutex_lock(&worker_mutex);
	list_for_each_entry_safe(act, safe, &worker_list, list) {
		if (act->op == LD_OP_UPDATE_LOCAL) {
			list_del(&act->list);
			free(act);
			log_debug("work_remove_update_local");
		}
	}
	pthread_mutex_unlock(&worker_mutex);
}
...
static void work_update_local_vgs(void)
{
	...
	/*
	 * Temporary work around to the issue where a lot of devices
	 * all appear together, pvscan is run for each of them to
	 * populate lvmetad, each pvscan triggers an update_local,
	 * and we end up calling this function a bunch of times in a row.
	 * We only really need/want one update_local when they are
	 * all done, and this is a rough approximation of that.
	 */
	if ((unsigned)time(NULL) - (unsigned)last_update_local <= 1) {
		sleep(2);
		work_remove_update_local();
	}
	last_update_local = time(NULL);
