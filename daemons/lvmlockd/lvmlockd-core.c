...
static void get_vg_lm_info(struct lockspace *ls)
{
	...
	response = daemon_reply_str(reply, "response", NULL);
	if (strcmp(response, "OK")) {
		log_error("get_vg_lm_info %s lvmetad response %s", ls->name, response);
		goto out;
	}
	...
	lock_args = dm_config_find_str(metadata, "metadata/lock_type", NULL);
	if (lock_args)
...
/* lockspaces_mutex is locked */
static struct lockspace *find_lockspace_uuid(char *vg_uuid)
{
	struct lockspace *ls;
...
static int add_lockspace_thread(char *ls_name, char *vg_name, char *vg_uuid,
...
static int start_lockspaces(uint64_t host_id)
{
	...
	for (cn = cn->child; cn; cn = cn->sib) {
		...
		rv = add_lockspace_thread(ls_name, lockspace.vg_name, lockspace.vg_uuid,
					  lockspace.lm_type, lockspace.lm_args,
...
static void client_recv_action(struct client *cl)
{
	...
	if (str && strcmp(host_id_str, "none"))
