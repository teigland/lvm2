...
static int res_lock(struct lockspace *ls, struct resource *r, struct action *act)
{
	...
	if (!version || version > r->version) {
		 ...
		 * flag, and provide the new seqno.  If lvmetad finds
		 ...
		 *
		 * r is global: tell lvmetad to set the global_invalid
		 * flag and provide the new seqno.  When commands see
		 * this flag returned from lvmetad, they will reread
		 * metadata from disk, update the lvmetad caches, and tell
		 * lvmetad to clear the global_invalid flag.
		 */
