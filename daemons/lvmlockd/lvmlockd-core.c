			     ...
			     int *retry_out)
{
	...
	list_for_each_entry_safe(act, safe, &r->actions, list) {
		...
		if (lk->mode != act->mode) {
			/* TODO: support lock mode conversion by automatically
			   queuing separate unlock+lock actions? */
			act->result = -EEXIST;
			list_del(&act->list);
			add_client_result(act);
		} else {
	...
	list_for_each_entry_safe(act, safe, &r->actions, list) {
		...
		if (lk->mode != act->mode) {
			/*
			 * Existing persistent lock needs to be unlocked first.
			 * Perhaps add an option to do an internal unlock+lock
			 * to change the mode of an existing persistent lock.
			 */
			act->result = -EEXIST;
			list_del(&act->list);
			add_client_result(act);
