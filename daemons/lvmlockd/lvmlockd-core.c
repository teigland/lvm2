	...
	for (cn = cn->child; cn; cn = cn->sib) {
		...
		if (!gl_type_static) {
			...
			if (!gl_use_dlm && lm_type == LD_LM_DLM) {
				log_debug("start_lockspaces set gl_use_dlm for %s",
					  lockspace.vg_name);
				gl_use_dlm = 1;
				gl_use_sanlock = 0;
			}
...
static void *worker_thread_main(void *arg_in)
{
	...
	while (1) {
		...
		log_debug("W op %s vg_name \"%s\"", op_str(act->op), act->vg_name);
		...
		if (act->flags & LD_AF_SEARCH_LS) {
			...
			if (is_local) {
				log_debug("worker: vg is local \"%s\"", act->vg_name);
				act->result = -ELOCALVG;
				add_client_result(act);
				continue;
			}
 ...
 retry:
	...
	if (!ls) {
		...
		if (act->op == LD_OP_UPDATE && act->rt == LD_RT_VG) {
			log_debug("add_lock_action no lockspace ignored for vg update");
			return -ENOLS;
			...
			log_debug("add_lock_action lockspace not found pass to worker");
			act->flags |= LD_AF_SEARCH_LS;
			add_work_action(act);
			return 0;
...
static void client_recv_action(struct client *cl)
{
	...
	if (!strcmp(req_name, "lock_vg") && vg_name &&
		...
		log_debug("skip lock_vg for local_vg %s", vg_name);
