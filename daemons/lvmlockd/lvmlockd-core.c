...
static int daemon_local_locking;
...
static uint64_t daemon_host_id;
...
static void client_recv_action(struct client *cl)
{
	...
	if ((result == -EOTHERVG) || (result == -ELOCALVG && !daemon_local_locking)) {
	...
	case LD_OP_LOCK:
		if (lls)
			rv = add_local_lock_action(lls, act);
		else
...
int main(int argc, char *argv[])
{
	...
	// use getopt_long
	while ((opt = getopt(argc, argv, "?fhVTs:i:y:")) != EOF) {
		switch (opt) {
		case 'T':
			daemon_test = 1;
			break;
