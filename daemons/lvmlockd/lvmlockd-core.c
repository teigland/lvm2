...
static int gl_ls_name(char *ls_name)
{
	if (gl_use_dlm)
		...
		log_error("gl_ls_name unknown gl_use type");
		return -1;
	}
}
	...
	while (1) {
		...
		if (act->flags & LD_AF_SEARCH_LS) {
			...
			if (is_ls) {
				...
				log_error("worker: lockspace not found %s", ls_name);
				act->result = -ENOLS;
				add_client_result(act);
			}
		...
		default:
			log_error("worker: unknown op %d", act->op);
			act->result = -EINVAL;
			add_client_result(act);
			break;
		};
...
static void client_send_result(struct client *cl, struct action *act)
{
	...
	if (act->result == -ENOLS) {
		...
		if (!gl_lsname_sanlock[0] && !gl_lsname_dlm[0])
 ...
 retry:
	...
	if (!ls) {
		...
		if (act->flags & LD_AF_SEARCH_LS) {
			...
			log_error("add_lock_action lockspace not found repeated %s", ls_name);
			return -ENOLS;

		} else if (act->op == LD_OP_LOCK && act->rt == LD_RT_GL && gl_use_dlm) {
			log_debug("add_lock_action add dlm global lockspace");
			act->flags |= LD_AF_SEARCH_LS;
			add_dlm_global_lockspace(NULL);
			goto retry;

		} else if (act->op == LD_OP_LOCK) {
			log_debug("add_lock_action lockspace not found pass to worker %s", ls_name);
			act->flags |= LD_AF_SEARCH_LS;
			list_del(act->list);
			add_work_action(act);
			return 0;
			...
			log_error("add_lock_action lockspace not found %s", ls_name);
			return -ENOLS;
		}
	}
