 ...
 *
 * vgcreate --lock-init lm_type vg_name devs
 * vgchange --lock-init lm_type vg_name devs
 *
 * Test:
 ...
 * lvmlock --lock-init sanlock --lock-args /dev/foo/leases --vg foo
 *
 ...
 *
 * (equivalent: lvcreate --lock-init lm_type -n lv_name vg_name)
 *
 * Test:
 ...
 * lvcreate --lock-init none -n lv_name vg_name
 * Later clusterize the non-clustered lv within the clustered vg:
 * lvchange --lock-init lm_type -n lv_name vg_name
 *
 * ----
 *
			     ...
			     int *retry_out)
{
	 ...
	 * first check if the lock being requested already exists
	 * (is this needed? when?)
	 */
	...
	list_for_each_entry_safe(act, safe, &r->actions, list) {
		...
		if (lk->mode < act->mode) {
			/* TODO: error? upconvert? unlock+lock? */
		}
	...
	/*
	 * persistent locks are not owned by a client,
	 * so any existing with matching mode is good.
	...
	/*
	 * non-persistent locks among different clients,
	 *
	 * if incompat mode is held, leave the act on r->actions
	 *
...
static int init_vg(struct action *act)
{
	...
	if (act->lm_type == LD_LM_SANLOCK)
		rv = lm_init_vg_sanlock(ls_name, act->lm_args);
...
static void client_send_result(struct client *cl, struct action *act)
{
	...
	if (act->op == LD_OP_INIT && act->rt == LD_RT_LV)
		res = daemon_reply_simple("OK",
					  "op = %d", act->op,
					  "op_result = %d", act->result,
					  "lock_args = %s", act->lm_args,
					  NULL);
	else
		res = daemon_reply_simple("OK",
					  "op = %d", act->op,
...
static int add_lock_action(struct action *act)
{
	...
	if (!ls) {
		...
		log_error("global lockspace not found %s", ls_name);
		return -ENOENT;
	}
