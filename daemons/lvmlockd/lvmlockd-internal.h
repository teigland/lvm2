...
/* TODO: make vb.version 16 bits, vb.flags 16 bits, vb.r_version 32 bits */

#define VAL_BLK_VERSION 0x00010001
...
int lm_unlock_dlm(struct lockspace *ls, struct resource *r, uint64_t version, uint32_t lmuf_flags);
...
int lm_lock_sanlock(struct lockspace *ls, struct resource *r, int ld_mode,
		    char *lv_args, uint64_t *version, int *retry);
int lm_convert_sanlock(struct lockspace *ls, struct resource *r,
		       int ld_mode, uint64_t version);
int lm_unlock_sanlock(struct lockspace *ls, struct resource *r, uint64_t version, uint32_t lmuf_flags);
