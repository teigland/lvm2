			  ...
			  const char *gl_mode, const char *vg_mode)
{
	...
	if (!strcmp(gl_mode, "enable"))
		opts = "enable";
	else if (!strcmp(gl_mode, "disable"))
		opts = "disable";
...
int dlock_gl_able(struct cmd_context *cmd, const char *vg_name, const char *able)
{
	daemon_reply reply;
	const char *able_gl;
	int result;
	...
	log_debug("dlock_gl_able %s %s", vg_name, able);
	...
	if (!ret) {
		log_error("Global lock %s result %d", able, result);
	}
