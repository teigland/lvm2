...
static int _lvmlockd_handle_reply(daemon_reply reply, const char *action, const char *object)
{
	...
	if (!strcmp(daemon_reply_str(reply, "response", ""), "OK")) {
		...
		log_error("lvmlockd reply op_result %d", result);
		  ...
		  const char *opts)
{
	...
	if (vg_name && lv_name) {
		...
		log_debug("dlock %s vg %s reply %d", id, vg_name, result);
		...
		log_debug("dlock %s reply %d", id, result);
	}
