...
static int _lvmlockd_result(daemon_reply reply, int *result, uint32_t *result_flags)
{
	...
	log_debug("lvmlockd_result %d", reply_result);
	...
	*result = reply_result;
	...
	if (reply_flags) {
		log_debug("lvmlockd_result flags %s", reply_flags);
			      ...
			      const char *lock_lv_name)
{
	...
	lp.lv_name = dm_pool_strdup(cmd->mem, lock_lv_name);
 ...
 *
 * TODO:
 * The update will not work because it's before the start.
 * Avoid reporting an error in that case, and perhaps save
 * the version and have start_vg acquire the vg lock to
 * save the version.
 */
 ...
 * lvmlockd can query for other live hosts in the lockspace,
 * and could use a sanlock host_message to tell them to leave.
 * We could wait for some timeout for all other hosts to leave
 * and then fail the vgremove if they haven't.
 *
 * When other hosts are gone, leave the sanlock lockspace ourself,
 * lvremove the sanlock lv, and continue with the standard vgremove.
 */
...
int dlock_free_vg_dlm(struct cmd_context *cmd, struct volume_group *vg)
{
	...
	/* TODO: ask any other hosts to leave the lockspace? */
			  ...
			  const char *lock_type, const char **lock_args_ret)
{
	...
	if (!ret) {
		log_error("lvmlockd init_lv error.");
		goto out;
	}
