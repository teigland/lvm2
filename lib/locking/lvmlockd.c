...
int dlock_init_vg(struct cmd_context *cmd, struct volume_group *vg)
{
	...
	/*
	 * TODO:
	 * If we set lock_args here, then we need to write the vg again?
	 * vg->lock_args = dm_pool_strdup(cmd->mem, lvmlocks_name);
	 */
...
/*
 * TODO: make lv lock_args for sanlock lvmlocks:offset
 * where lvmlocks is copied from the vg lock_args
 */
