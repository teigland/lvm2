...
static int _lvmlockd_handle_reply(daemon_reply reply, const char *action, const char *object)
{
	...
	if (!strcmp(daemon_reply_str(reply, "response", ""), "OK")) {
		...
		result = daemon_reply_int(reply, "op_result", -1000);
		if (result == -1000) {
			log_error("No op_result found in daemon reply");
			return 0;
		}
		...
		log_error("lvmlockd reply %s %s op_result %d", action, object, result);
		  ...
		  const char *opts)
{
	...
	if (vg_name && lv_name) {
		...
		result = _lvmlockd_handle_reply(reply, req_name, lv_name);

		log_debug("dlock %s vg %s lv %s reply %d",
			  req_name, vg_name, lv_name, result);
		...
		result = _lvmlockd_handle_reply(reply, req_name, vg_name);

		log_debug("dlock %s vg %s reply %d", req_name, vg_name, result);
		...
		result = _lvmlockd_handle_reply(reply, req_name, NULL);

		log_debug("dlock %s reply %d", req_name, result);
	}
