...
/*
 * process_each_vg will call dlock_vg(cmd, vg_name, NULL, 0);
	     ...
	     const char *def_mode, uint32_t flags)
{
	...
	const char *mode, *cmd_mode;
	const char *opts = NULL;

	cmd_mode = arg_str_value(cmd, locklv_ARG, NULL);
	if (!cmd_mode)
		mode = def_mode;
