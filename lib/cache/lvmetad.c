 ...
 *
 * Check if lvmetad's global invalid flag has been set by lvmlockd.
 * If so, reread all metadata from disk, update it in lvmetad,
 * and clear lvmetad's global invalid flag.
 */

void lvmetad_validate_dlock_global(struct cmd_context *cmd, int force)
{
	daemon_reply reply;
	int global_invalid;
	...
	/*
	 * TODO: replace pv_list with a new request that just returns
	 * the global_invalid information since that's all we need here.
	 */
	reply = daemon_send_simple(_lvmetad, "pv_list",
				   "token = %s", "skip",
				   NULL);

	if (reply.error)
		log_error("lvmetad_validate_dlock_global pv_list lvmetad error %d", reply.error);

	if (strcmp(daemon_reply_str(reply, "response", ""), "OK"))
		log_error("lvmetad_validate_dlock_global pv_list lvmetad bad response");

	global_invalid = !!dm_config_find_node(reply.cft->root, "global_invalid");

	daemon_reply_destroy(reply);

	if (!global_invalid) {
		/* TODO FIXME: remove this warn */
		log_warn("lvmetad global cache is good");
		return;
	}

 do_scan:
	/* TODO FIXME: remove this warn */
	log_warn("lvmetad global cache is stale, refreshing");

	/* TODO: does this fully rebuild the cache? */
	...
	if (reply.error)
		log_error("lvmetad_validate_dlock_global set_global_info lvmetad error %d", reply.error);

	if (strcmp(daemon_reply_str(reply, "response", ""), "OK"))
		log_error("lvmetad_validate_dlock_global set_global_info lvmetad bad response");
