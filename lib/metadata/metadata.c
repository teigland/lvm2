...
disk_read:
	...
	if (update_lvmetad) {
		correct_vg->cft_precommitted = export_vg_to_config_tree(correct_vg);
		if (!lvmetad_vg_update(correct_vg))
			log_error("Failed to update lvmetad with new vg version");

		if (correct_vg->cft_precommitted) {
				    ...
				    uint64_t status)
{
	...
	if ((status & CLUSTERED) &&
	    (vg_is_clustered(vg)) && !locking_is_clustered() && !lvmlockd_connected()) {
			       ...
			       uint64_t status_flags, uint32_t misc_flags)
{
	...
	if (vg_is_clustered(vg) && !locking_is_clustered() && !lvmlockd_connected()) {
