	     ...
	     const char *def_mode, uint32_t flags)
{
	...
	if (result == -ENOLS || result == -ESTARTING) {
		...
		if (strcmp(mode, "sh")) {
			/*
			 * For ENOLS, we may want to continue just through
			 * vg_read to see if the vg can be read, then fail.
			 * dlock_vg_verify would catch this case and return
			 * an error.
			 */
			if (result == -ENOLS)
				log_error("VG \"%s\" lock not found", vg_name);
			else if (result == -ESTARTING)
				log_error("VG \"%s\" lock start in progress", vg_name);
			else
 ...
 *
 * TODO2:
 * . When dlock_vg ex does not find a lockspace for the vg, instead
 *   of failing immediately after dlock_vg, we may want to continue
 *   through vg_read, and then fail in dlock_vg_verify.  If we
 *   continue through vg_read, we could report whether the vg could
 *   even be found for reading, in addition to reporting that the
 *   vg could not be locked.  It provides a potentially more useful
 *   error to know that the vg could not be read in addition to the
 *   fact it could not be locked.
 *
 *   dlock_vg(cmd, vg_name, "ex", &result);
 *   . sets result to ENOLS (lockspace not found)
 *   . we don't return immediately after this error...
 *   vg_read();
 *   . reports an error if the vg can not be read
 *   dlock_vg_verify(cmd, vg, result);
 *   . if we get this far, we see that the vg could be read,
 *     but the lockspace was not found (ENOLS).  We fail at
 *     this point with more a more useful error report.
 */
...
int dlock_lv_name(struct cmd_context *cmd, struct volume_group *vg,
		  ...
		  const char *def_mode, uint32_t flags)
{
	const char *ac_mode = NULL;
	const char *mode = NULL;
	const char *opts = NULL;
	activation_change_t ac;
	uint32_t result_flags;
	int result;
	int ret;
	...
	/* -ae == --lock-lv ex, -as == --lock-lv sh */
	ac = (activation_change_t)arg_uint_value(cmd, activate_ARG, CHANGE_AY);
	if (ac == CHANGE_AE)
		ac_mode = "ex";
	else if (ac == CHANGE_AS)
		ac_mode = "sh";

	mode = arg_str_value(cmd, locklv_ARG, NULL);
	if (mode && def_mode &&
	    (mode_compare(mode, "sh") < 0) &&
	    !find_config_tree_bool(cmd, global_allow_unsafe_lock_modes_CFG, NULL)) {
		log_error("Disallowed lock-lv mode \"%s\"", mode);
		return 0;
	}

	if (mode && ac_mode && strcmp(mode, ac_mode)) {
		log_error("Different LV lock modes from activation %s and lock-lv %s", ac_mode, mode);
		return 0;
	}
...
/*
 * For a thin pool and all its thin volumes, one ex dlock is used.
 * It is the one specified in metadata of the pool data lv.
 */

static int dlock_lv_thin(struct cmd_context *cmd, struct logical_volume *lv,
			 const char *def_mode, uint32_t flags)
{
	...
	if (lv_is_thin_volume(lv)) {
		...
		log_error("Lock on incorrect thin lv type %s/%s", lv->name);
		return 0;
	}

	if (!pool_lv) {
		/* This should not happen. */
		log_error("Cannot find thin pool for %s/%s", lv->vg->name, lv->name);
		return 0;
	}
	...
	return dlock_lv_name(cmd, pool_lv->vg, pool_lv->name, pool_lv->lock_args,
			     def_mode, flags);
}
...
int dlock_lv(struct cmd_context *cmd, struct logical_volume *lv,
	     const char *def_mode, uint32_t flags)
{
	if (!dlock_type(lv->vg->lock_type))
		return 1;

	if (lv_is_thin_type(lv))
		return dlock_lv_thin(cmd, lv, def_mode, flags);
