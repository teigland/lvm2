		    ...
		    const char *vg_lock_type)
{
	...
	log_debug("dlock_gl_create %s", command_name(cmd));
	...
	if (result == -ENOLS) {
		...
		if ((result_flags & LD_RF_NO_GL) &&
		    ...
		    !strcmp(vg_lock_type, "sanlock")) {
			log_warn("Enabling sanlock global lock");
			lvmetad_validate_dlock_global(cmd, 1);
			return 1; /* success */
		}
...
int dlock_gl(struct cmd_context *cmd, const char *def_mode, uint32_t flags)
{
	...
	if (result == -ENOLS) {
		 ...
		 */
		if ((result_flags & LD_RF_NO_GL) && !strcmp(mode, "sh")) {
