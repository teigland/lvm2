			 ...
			 const char *def_mode, uint32_t flags)
{
	...
	if (lv_is_thin_volume(lv)) {
		pool_seg = first_seg(lv);
		pool_lv = pool_seg->pool_lv;
		data_seg = first_seg(pool_lv);
		data_lv = seg_lv(data_seg, 0);
		...
		log_error("Thin pool data lv %s cannot be locked directly.", lv->name);
		return 0;

	} else if (lv_is_thin_pool_metadata(lv)) {
		/* This should not happen? */
		log_error("Thin pool metadata lv %s cannot be locked directly.", lv->name);
		return 0;

	} else {
		/* This should not happen? */
		log_error("Unknown type of thin lv %s", lv->name);
		return 0;
	}

	/*
	 * When lvconvert creates the thin pool, the lock_args are
	 * in the data lv.  When lvcreate creates the thin pool,
	 * the lock_args are in the pool lv.
	 * FIXME: probably better to have lock_args in a consistent place.
	 */

	if (pool_lv->lock_args)
		lock_args = pool_lv->lock_args;
	else if (data_lv->lock_args)
		lock_args = data_lv->lock_args;

	/*
	 * If already locked, a new lock op is a no-op.
	 * Only unlock when the pool is no longer active.
	 */

	if (!strcmp(def_mode, "un") && pool_is_active(pool_lv))
		return 1;

	return dlock_lv_name(cmd, lv->vg, pool_lv->name, lock_args, def_mode, flags);
}
