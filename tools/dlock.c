		    ...
		    const char *vg_lock_type)
{
	...
	if (result < 0) {
		log_error("Locking error %d for global lock", result);
		return 0; /* failure */
	}
	     ...
	     const char *def_mode, uint32_t flags)
{
	...
	if (result < 0) {
		...
		log_error("Locking error %d for VG %s", result, vg_name);
		return 0; /* failure */
	}
		  ...
		  const char *def_mode, uint32_t flags)
{
	...
	if (result < 0) {
		log_error("Locking error %d for LV %s/%s", result, vg->name, lv_name);
		return 0;
	}
