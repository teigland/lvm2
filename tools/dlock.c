	     ...
	     const char *def_mode, uint32_t flags)
{
	...
	if (result == -EOTHERVG) {
		log_warn("VG %s skipped: owned by other system id", vg_name);
		return 0; /* failure */
	}
...
int dlock_vg_verify(struct cmd_context *cmd, struct volume_group *vg)
{
	if (vg->system_id && vg->system_id[0] &&
		...
		log_error("Skip VG %s with system id \"%s\" for local system id \"%s\"",
			  vg->name, vg->system_id, cmd->hostname);
		goto fail;
	}

	if (vg_is_clustered(vg) && find_config_tree_bool(cmd, global_use_lvmlockd_CFG, NULL)) {
		log_error("Skip VG %s requiring clvmd while using lvmlockd", vg->name);
		goto fail;
	}
