			 ...
			 struct lvcreate_params *lp)
{
	...
	if (!lv_init_lock_args(cmd, vg, lp->lv_name, lock_type, &lock_args)) {
	...
	if (!dlock_lv_name(cmd, vg, lp->lv_name, lock_args, def_mode, LD_LV_PERSISTENT)) {
		log_error("Failed to lock lv");
		lv_free_lock_args(cmd, vg, lp->lv_name, lock_type, lock_args);
		return 0;
	}
...
static void _free_lv_lock(struct cmd_context *cmd, struct volume_group *vg,
			  struct lvcreate_params *lp)
{
	dlock_lv_name(cmd, vg, lp->lv_name, lp->lock_args, "un", LD_LV_PERSISTENT);
	lv_free_lock_args(cmd, vg, lp->lv_name, lp->lock_type, lp->lock_args);
}
