			 ...
			 struct lvcreate_params *lp)
{
	 ...
	 * be no-ops.
	 */
	if (lp->thin && !seg_is_thin_pool(lp))
		return 1;
	...
	if (arg_count(cmd, locktype_ARG))
		lock_type = dm_pool_strdup(cmd->mem, arg_str_value(cmd, locktype_ARG, NULL));
	else if (vg->lock_type)
		lock_type = dm_pool_strdup(cmd->mem, vg->lock_type);

	if (!dlock_init_lv_lock_args(cmd, vg, lv_name, lock_type, &lock_args)) {
	...
	def_mode = is_change_activating(lp->activate) ? "ex" : "na";

	if (!dlock_lv_name(cmd, vg, lv_name, lock_args, def_mode, DL_LV_PERSISTENT)) {
		log_error("Failed to lock lv");
		dlock_free_lv_lock_args(cmd, vg, lv_name, lock_type, lock_args);
		return 0;
	}
			  ...
			  struct lvcreate_params *lp)
{
	...
	dlock_lv_name(cmd, vg, lv_name, lp->lock_args, "un", DL_LV_PERSISTENT);
	dlock_free_lv_lock_args(cmd, vg, lv_name, lp->lock_type, lp->lock_args);
}
