			      ...
			      struct volume_group *vg)
{
	...
	if (vg->system_id && vg->system_id[0] && !arg_count(cmd, force_ARG)) {
			   ...
			   void *handle __attribute__((unused)))
{
	...
	if (arg_count(cmd, lockstart_ARG)) {
		if (!_vgchange_lock_start(cmd, vg))
			return_ECMD_FAILED;
	} else if (arg_count(cmd, lockstop_ARG)) {
...
static int dlock_vgchange(struct cmd_context *cmd, int argc, char **argv)
{
	...
	if (arg_count(cmd, systemid_ARG) || arg_count(cmd, locktype_ARG))
	...
	if (arg_count(cmd, systemid_ARG) && arg_count(cmd, force_ARG))
		cmd->command->flags |= DLOCK_VG_NA;

	/*
	 * --lock-gl enable|disable are special; they are commands
	 *  themselves and are not options for another command.
	 */

	mode = arg_str_value(cmd, lockgl_ARG, NULL);
	if (mode && (!strcmp(mode, "enable") || !strcmp(mode, "disable")))
		return 1;
	...
	if (!argc || arg_tag_count(argc, argv) ||
	    arg_count(cmd, lockstart_ARG) || arg_count(cmd, lockgl_ARG)) {
		if (!dlock_gl(cmd, "sh", DL_GL_RENEW_CACHE))
			return ECMD_FAILED;

	} else if (arg_count(cmd, systemid_ARG) || arg_count(cmd, locktype_ARG)) {
...
int vgchange(struct cmd_context *cmd, int argc, char **argv)
{
	...
	/*
	 * Special case to enable or disable the global lock in a sanlock vg:
	 * vgchange --lock-gl enable|disable vg_name
	 * This only ever wants a single explicit vg, so toollib
	 * processing is not wanted.  This is the only case where the
	 * lockgl arg implies a command itself.  It is otherwise a
	 * modifier for other commands.
	 */
	if (arg_count(cmd, lockgl_ARG)) {
		const char *able = arg_str_value(cmd, lockgl_ARG, NULL);
		if (!strcmp(able, "enable") || !strcmp(able, "disable")) {
			if (argc != 1 || arg_tag_count(argc, argv)) {
				log_error("Enable/disable global lock in a single VG");
				return ECMD_FAILED;
			}
			return dlock_gl_able(cmd, argv[0], able);
		}
		/*
		 * When the lock-gl arg is not enable/disable,
		 * then we fall through to normal processing where the
		 * lock-gl option is just a modifier for another command.
		 */
	}
