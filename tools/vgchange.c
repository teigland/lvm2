...
static int _vgchange_locktype(struct cmd_context *cmd,
			      struct volume_group *vg)
{
	const char *lt = arg_str_value(cmd, locktype_ARG, NULL);
	...
	if (!vg_set_lock_type(vg, lt, 1))
		return 0;

	if (!dlock_init_vg_lock_args(cmd, vg)) {
		log_error("Failed to initialize VG %s lock args for lock type %s", vg->name, lt);
		return 0;
	}
...
static int dlock_vgchange(struct cmd_context *cmd, int argc, char **argv)
{
	...
	/* Chaning system_id or lock_type must only be done on explicitly named vgs. */
